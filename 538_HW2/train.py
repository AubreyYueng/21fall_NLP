...
def train(model: nn.Module,
    ...
    for epoch in range(num_epochs):
        ...
        if validation_accuracy > best_epoch_validation_accuracy:
            ...
            if hasattr(classifier, "module") else classifier.state_dict()
            }
...
if __name__ == '__main__':
    ...
    if args.load_serialization_dir:
        ...
        if args.model_name == "main":
            ...
            config = {"pretrained_model_path": args.base_model_dir, "input_dim": input_dim,
    ...
    if args.model_name == "main" and args.pretrained_embedding_file:
        embeddings = load_glove_embeddings(args.pretrained_embedding_file,
                                           args.embedding_dim,
                                           vocab_id_to_token)
        classifier._embedding_layer = nn.Embedding.from_pretrained(torch.Tensor(embeddings).to(device),freeze=False)
